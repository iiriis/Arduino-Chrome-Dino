...
bool drawPacman(byte position, char* tupp, char* tlw, unsigned int score) {
  bool col = false;
  char us = tupp[hhp];
  char ls = tlw[hhp];
  byte upper, lower;
...
void loop(){
  ...
  if (!playing) {
    drawPacman((blink) ? hpo : hpos, tupp, tlw, distance >> 3);
    if (blink) {
      lcd.setCursor(0,0);
 ...
 // advterrain(tupp, nterrType == tub ? sts : ste);
  ...
  if (drawPacman(hpos, tupp, tlw, distance >> 3)) 
    playing = false; 
   else 
   {
